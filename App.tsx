        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="px-3 py-0.5 bg-pink-500 text-white text-[10px] font-black rounded-full tracking-widest uppercase">Gemini 3 Flash</span>
              <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em]">AR Debug Node v3.1.0</p>
            </div>
          </div>
        </div>
...
Pin 2 -> VCC (5V)"
            ...
            <button
              ...
              className={`w-full anime-btn py-5 rounded-3xl font-black flex items-center justify-center gap-4 text-white text-xl shadow-[0_8px_0_#d14d72] active:shadow-none active:translate-y-[8px] mt-8 ${!state.image || state.isAnalyzing ? 'bg-slate-300 shadow-[0_8px_0_#94a3b8]' : 'bg-pink-500'
                }`}
            >
              {state.isAnalyzing ? (
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
              ...
              {state.result && (
                <div className="flex items-center gap-4 bg-slate-50 p-3 rounded-3xl border-2 border-slate-100">
                  ...
                  <div className={`px-5 py-2 rounded-2xl border-4 flex items-center gap-3 transition-all ${state.result.health_score > 80 ? 'bg-green-50 border-green-100 text-green-700' : 'bg-yellow-50 border-yellow-100 text-yellow-700'
                    }`}>
                      ...
                      {state.result.faults.length === 0 ? (
                        ...
                        state.result.faults.map(fault => (
                          <div
                            ...
                            className={`group cursor-pointer p-6 rounded-[1.5rem] border-4 transition-all duration-300 ${selectedFaultId === fault.id
                                ? 'bg-pink-50 border-pink-400 shadow-xl -translate-y-1'
                                : 'bg-white border-slate-100 hover:border-pink-200 hover:shadow-lg'
                              }`}
                          >
                            <div className="flex items-center gap-4">
                              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 border-2 ${fault.severity === 'critical' ? 'bg-red-500 border-red-400 text-white' : 'bg-yellow-400 border-yellow-300 text-black'
                                }`}>
      ...
      <footer className="fixed bottom-8 left-0 right-0 p-4 flex justify-center pointer-events-none z-50">
        <div className="glass-morphism px-8 py-3 rounded-full border-4 border-white shadow-2xl pointer-events-auto flex items-center gap-8 animate-in slide-in-from-bottom-4 duration-1000">
          ...
          <div className="flex items-center gap-2 text-pink-500">
            ...
            <p className="text-[11px] font-black tracking-widest uppercase italic">Gemini 3 Neuro-Core v1.0</p>
          </div>
          <div className="h-6 w-[2px] bg-slate-200"></div>
